...
def add_artist(artist):
    ...
    if len(artist.replace(' ', '')) < 2:
        return Response(f"Seems like most artists should contain more than 2 characters!  Should probably try that one again.")
...
def add_album(artist, album):
    ...
    album = album.title()
    if len(artist.replace(' ', '')) < 2:
        return Response(f"Seems like most artists should contain more than two characters!")
    if len(album.replace(' ', '')) < 2:
        return Response(f"Seems like this album should have more than two characters in it!")
    database = DbFunctions()
    resp = database.add_artist_albums(artist, [album])
    database.close()
    if resp == SUCCESS_NO_RESPONSE:
        return Response(f"Added {album} for {artist} to the album database!")
    elif resp == NO_ITEM_TO_ADD:
        return Response(f"You're going to need to specify the album!")
    elif resp == NOT_FOUND:
        return Response(f"The artist {artist} wasn't found in the database!  It needs to exist before we can add albums to it!")
...
def add_track(artist, album, track):
    ...
    if resp == SUCCESS_NO_RESPONSE:
        ...
        return Response(f"You're going to need to specify a track to add!")
    elif resp == NOT_FOUND:
        return Response(f"The album {album} wasn't found in the database!  It needs to exist before we can add tracks to it!")
...
def add_lyrics(artist, album, track):
    ...
    if resp == SUCCESS_NO_RESPONSE:
        ...
        return Response(f"The track {track} wasn't found in the database!  It needs to exist before we can add lyrics to it!")
    elif resp == NO_ITEM_TO_ADD:
        return Response(f"We found 'lyrics' in the JSON, but you need to specify some actual lyrics!")
...
def update_artist(artist):
    ...
    if 'artist' not in request.json:
        return Response(f"You need to specify a new artist name in a JSON with the key 'artist'!")
...
def update_album(artist, album):
    ...
    album = album.title()
    if len(artist.replace(' ', '')) < 2:
        return Response("Seems like most artists should contain more than two characters!")
    if len(album.replace(' ', '')) < 2:
        return Response("Seems like this album should have more than two characters in it!")
    if "album" not in request.json:
        return Response("You need to specify a new album name in a JSON with the key 'album'!")
    engine = create_engine("postgresql+pg8000://postgres:@localhost:5454/test").execution_options(autocommit=True)
    database = Session(engine)
    ...
    if check == None:
        database.close()
        engine.dispose()
        return Response(f"I can't find the original album!")
    else:
...
def update_track(artist, album, track):
    ...
    if "track" not in request.json:
        return Response("You need to specify a new track name in a JSON with the key 'track'!")
    engine = create_engine("postgresql+pg8000://postgres:@localhost:5454/test").execution_options(autocommit=True)
    database = Session(engine)
    ...
    if check == None:
        database.close()
        engine.dispose()
        return Response(f"I can't find that track!")
    else:
