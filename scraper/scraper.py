...
def get_albums(artists: list = []):
    ...
    if resp == NOT_FOUND:
        logging.critical(f"The artist {artist} was not found while trying to add albums to the database!")
    else:
...
def find_album_list_in_page(soup):
    ...
    if finder != None:
        return finder
    logging.debug("Album list helper failed out")
    return None
...
def find_track_list(soup):
    ...
    if finder != None:
        return finder
    logging.debug("Track list helper failed out")
    return None
